...
@kopf.timer("networking.k8s.io", "v1", "ingresses", interval=_RESYNC_INTERVAL)
def reconcile_ingress(spec, meta, namespace, name, patch, logger, memo: kopf.Memo, **_):
    ...
    patch.metadata.annotations = {
        STATUS_ANNOTATION: ("ready: " + "; ".join(messages)) if messages else "no-hosts"
    }
...
@kopf.timer("dns.rarosalion.github.io", "v1", "dnsaliases", interval=_RESYNC_INTERVAL)
